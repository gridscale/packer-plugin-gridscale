...
// Note this easyssh package is a copy of "github.com/hypersleep/easyssh" repository
// It was copied for being modified to be compatible with gridscale packer.
package easyssh
...
func (ssh_conf *MakeConfig) Scp(sourceFile string, etargetFile string) error {
	...
	if err := session.Run(fmt.Sprintf("mkdir -p \"%s\" && scp -tr \"%s\"", filepath.Dir(etargetFile), etargetFile)); err != nil {
		return err
	}
